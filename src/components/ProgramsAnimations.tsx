...
import { FadeUp, StaggerContainer, StaggerItem, ScaleUp } from './ScrollAnimations';
...
export default function ProgramsAnimations({ programs }: { programs: Program[] }) {
  const inPersonPrograms = programs.filter(p => p.id === 'personal-training' || p.id === 'semi-private');
  const onlinePrograms = programs.filter(p => !['personal-training', 'semi-private'].includes(p.id));
